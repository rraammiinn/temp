<template>
  ...
  <div class="main">
    ...
    <tg-scrollable :all-messages="allChatMessages" :other-id="props.otherId" :get-previous-messages="getPreviousMessages" :get-next-messages="getNextMessages" :start-enabled="startEnabled" :end-enabled="endEnabled"></tg-scrollable>
  ...
  //   if(e.target.scrollTop==0 && startEnabled.value){
  //     previousScrollHeight=e.target.scrollHeight
  //     getPreviousMessages(e);console.log('start');}
  //   else if(e.target.scrollTop+e.target.clientHeight==e.target.scrollHeight && endEnabled.value){
  //     getNextMessages();console.log('end');}
  //   else{
  //     showGoToBottom.value = (endEnabled.value || e.target.scrollHeight - e.target.scrollTop > 5000) && (startScrollTop < e.target.scrollTop);
  //     startScrollTop=e.target.scrollTop;
  //   }
  // }
  
  async function getPreviousMessages({done}){console.log('-----------getPreviousMessages----------->>>>>>>>>>>')
    try{
      isTop=true
  ...
  async function getNextMessages({done}){console.log('------------getNextMessages---------->>>>>>>>>>>')
      try{
        isTop=false
  ...
  async function goToBottom(){
    ...
    subscribeToNewMessages()
    if(allChatMessages.value[props.otherId].messages.length<10)startEnabled.value=false;}
