<template>
<div :style="{alignSelf : (props.messageType=='channel') ? 'center' : (props.fromYou ? 'flex-end' : 'flex-start') }" style="border: solid;border-radius: .5rem;padding: .5rem;border-color: var(--tgBrown);margin: 1rem;max-width: 80%;margin-top: 1rem;margin-bottom: 3rem;width: min-content;">


  <div v-if="props.messageType=='group'" @click="showName = !showName" style="height: 3rem;min-width: 3rem;margin-left: -2rem;margin-top: -2rem;background-color: var(--tgBg);border-radius: .5rem;margin-bottom: 1rem;border-style: solid;border-color: var(--tgBrown);border-width: .2rem;width: fit-content;max-width: calc(100% + 1rem);display: flex;align-items: center;white-space: nowrap;">
    <img style="height: 100%;" :src="props.avatar" alt="">
  ...
  <v-card v-show="props.text" :created="props.time" :id="props.id" elevation="10" color="var(--tgBrown)" style="margin-bottom: 1.5rem;border-radius: .35rem;" :text="props.text">
      ...
      <div style="padding: 1rem;display: flex;justify-content: space-between;opacity: .5;font-size: .5rem;font-weight: bold;">
        ...
        <v-icon style="margin-left: .5rem;" v-if="props.fromYou" :icon="props.seen ? 'mdi-check-all' : 'mdi-check'"></v-icon>
      </div>
</div>
</template>
...
const props = defineProps(['seen','text','avatar','time','images','videos','audios','files','name','fromYou','fromOther','id','messageType'])
